...
#include <ChunkedArrayMerged.hpp>
#include <algorithm>
#include <vector>
#include <string>
#include <stdexcept>
...
TEST_CASE("ChunkedArray<int>: push_back matches vector", "[ChunkedArrayMerged][non-empty][push_back]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: emplace_back matches vector", "[ChunkedArrayMerged][non-empty][emplace_back]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<string>: emplace_back with args", "[ChunkedArrayMerged][non-empty][emplace_back]")
{
    ent::ChunkedArrayMerged<std::string, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: operator subscript read/write", "[ChunkedArrayMerged][non-empty][subscript]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<int>: at() with bounds checking", "[ChunkedArrayMerged][non-empty][at]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: ensure_size", "[ChunkedArrayMerged][non-empty][ensure_size]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: reserve", "[ChunkedArrayMerged][non-empty][reserve]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: clear", "[ChunkedArrayMerged][non-empty][clear]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: shrink_to_fit", "[ChunkedArrayMerged][non-empty][shrink_to_fit]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: back() and pop_back()", "[ChunkedArrayMerged][non-empty][back][pop_back]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<int>: iteration", "[ChunkedArrayMerged][non-empty][iterator]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<int>: reverse iteration", "[ChunkedArrayMerged][non-empty][iterator][reverse]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<int>: iterator random access", "[ChunkedArrayMerged][non-empty][iterator][random-access]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
...
TEST_CASE("ChunkedArray<int>: get_chunk_span", "[ChunkedArrayMerged][non-empty][get_chunk_span]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;

    SECTION("Empty container returns empty span")
    {
        auto span = chunked.get_chunk_span(0);
        REQUIRE(span.empty());
    }
    ...
    {
        chunked.ensure_size(100);
        auto span = chunked.get_chunk_span(100);
        REQUIRE(span.empty());
    }
    ...
    SECTION("Full chunk span")
    {
        ...
        auto span0 = chunked.get_chunk_span(0);
    ...
    SECTION("Partial last chunk span")
    {
        ...
        auto span1 = chunked.get_chunk_span(1);
        REQUIRE(span1.size() == 100);  // Only 100 elements in last chunk
    }
}

TEST_CASE("ChunkedArray<int>: iterators after reserve with surplus chunks", "[ChunkedArrayMerged][iterator][reserve]")
{
    ...
    // live chunks, so decrementing end() would step into an uninitialized chunk.
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;

    // Reserve several extra chunks, then fill only part of the first one.
    chunked.reserve(kChunkSize * 4);
    const size_t numElements = kChunkSize / 2;   // well within the first chunk
    for (size_t i = 0; i < numElements; ++i)
        chunked.push_back(static_cast<int>(i));

    REQUIRE(chunked.size() == numElements);
    REQUIRE(chunked.chunk_count() == 4);  // 4 allocated, only 1 live

    SECTION("Forward iteration visits exactly the live elements")
    {
        size_t count = 0;
        for (auto it = chunked.begin(); it != chunked.end(); ++it, ++count)
            REQUIRE(*it == static_cast<int>(count));
        REQUIRE(count == numElements);
    }

    SECTION("Reverse iteration: decrement from end() lands in the live chunk")
    {
        auto it = chunked.end();
        --it;
        REQUIRE(*it == static_cast<int>(numElements - 1));

        // Walk all the way back to begin
        size_t remaining = numElements - 1;
        while (it != chunked.begin())
        {
            --it;
            --remaining;
            REQUIRE(*it == static_cast<int>(remaining));
        }
        REQUIRE(remaining == 0);
    }
    ...
    {
        REQUIRE((chunked.end() - chunked.begin()) == static_cast<std::ptrdiff_t>(numElements));
    }

    SECTION("-= from end() crosses no uninitialized chunk")
    {
        auto it = chunked.end();
        it -= static_cast<std::ptrdiff_t>(numElements);
        REQUIRE(it == chunked.begin());
        REQUIRE(*it == 0);
    }
}

TEST_CASE("ChunkedArray<struct>: complex types", "[ChunkedArrayMerged][non-empty][complex]")
{
    struct Point3d {
        double x = 0, y = 0, z = 0;
        Point3d() = default;
    ...
    ent::ChunkedArrayMerged<Point3d, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: resize()", "[ChunkedArrayMerged][non-empty][resize]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: empty container", "[ChunkedArrayMerged][non-empty][iterator][empty]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    ...
    {
        ent::ChunkedArrayMerged<int, kChunkSize>::iterator defaultIt;
        ent::ChunkedArrayMerged<int, kChunkSize>::const_iterator defaultCIt;
        (void)defaultIt;
        (void)defaultCIt;
    }
}
...
TEST_CASE("ChunkedArray<int> iterator: operator== consistent with operator<", "[ChunkedArrayMerged][non-empty][iterator][ordering]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: position stays consistent", "[ChunkedArrayMerged][non-empty][iterator][position]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: decrement across chunk boundaries", "[ChunkedArrayMerged][non-empty][iterator][boundary]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: commutative n + it == it + n", "[ChunkedArrayMerged][non-empty][iterator][operator+]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: const_iterator from mutable iterator", "[ChunkedArrayMerged][non-empty][iterator][const]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    for (int i = 0; i < 300; ++i) chunked.push_back(i);
    ...
    {
        ent::ChunkedArrayMerged<int, kChunkSize>::iterator it = chunked.begin() + 100;
        ent::ChunkedArrayMerged<int, kChunkSize>::const_iterator cit = it;
        REQUIRE(*cit == *it);
    ...
    {
        ent::ChunkedArrayMerged<int, kChunkSize>::iterator it = chunked.begin();
        ent::ChunkedArrayMerged<int, kChunkSize>::const_iterator cit = it;
        it += 100;
...
TEST_CASE("ChunkedArray<int> iterator: std::distance", "[ChunkedArrayMerged][non-empty][iterator][std]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int> iterator: std::sort", "[ChunkedArrayMerged][non-empty][iterator][std]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
    std::vector<int> reference;
    const int N = static_cast<int>(kChunkSize) * 3 + 50;
...
TEST_CASE("ChunkedArray<int> iterator: std::find and std::reverse", "[ChunkedArrayMerged][non-empty][iterator][std]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<int>: pop_back then push_back write-pointer invariants", "[ChunkedArrayMerged][non-empty][pop_back][write_ptr]")
{
    ent::ChunkedArrayMerged<int, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<NoDflt>: pop_back does not require default constructor", "[ChunkedArrayMerged][non-default-constructible][pop_back]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<NoDflt>: clear destructs all elements", "[ChunkedArrayMerged][non-default-constructible][clear]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<NoDflt>: move construction transfers ownership", "[ChunkedArrayMerged][non-default-constructible][move]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> src;
        for (int i = 0; i < 10; ++i) src.emplace_back(i * 2);
        REQUIRE(NoDflt::live() == 10);

        ent::ChunkedArrayMerged<NoDflt, kChunkSize> dst(std::move(src));
        REQUIRE(src.empty());
        REQUIRE(dst.size() == 10);
...
TEST_CASE("ChunkedArray<NoDflt>: move assignment transfers ownership", "[ChunkedArrayMerged][non-default-constructible][move]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> src;
        ...
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> dst;
...
TEST_CASE("ChunkedArray<NoDflt>: destructor releases all elements", "[ChunkedArrayMerged][non-default-constructible][destructor]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray<NoDflt>: stress test push/pop cycles across chunk boundaries", "[ChunkedArrayMerged][non-default-constructible][pop_back][stress]")
{
...
TEST_CASE("ChunkedArray<NoDflt>: resize(n, value) with non-default-constructible value type",
          "[ChunkedArrayMerged][non-default-constructible][resize]")
{
    NoDflt::reset_counts();
    {
        ent::ChunkedArrayMerged<NoDflt, kChunkSize> chunked;
...
TEST_CASE("ChunkedArray: reserve then push_back reuses pre-allocated chunks",
          "[ChunkedArrayMerged][reserve][allocate_new_chunk][regression]")
{
    constexpr size_t CS = 4;
    ent::ChunkedArrayMerged<int, CS> arr;
...
TEST_CASE("ChunkedArray: reset then reserve then push_back reuses pre-allocated chunks",
          "[ChunkedArrayMerged][reserve][reset][allocate_new_chunk][regression]")
{
...
TEST_CASE("ChunkedArray: repeated reset+reserve+fill cycles stay coherent",
          "[ChunkedArrayMerged][reserve][reset][allocate_new_chunk][regression][stress]")
{
...
TEST_CASE("ChunkedArray: get_chunk_span returns correct size after reserve with surplus chunks",
          "[ChunkedArrayMerged][get_chunk_span][reserve][regression]")
{
    constexpr size_t CS = 4;
