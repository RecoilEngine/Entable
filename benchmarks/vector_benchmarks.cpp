// Benchmarks: entable::ChunkedArray vs recoil::ChunkedArray vs std::vector
...
namespace rec = recoil;
namespace mrg = entable;  // ChunkedArrayMerged lives in namespace entable
...
template <typename T>
static void BM_ChunkedArrayRecoil_PushBack(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        rec::ChunkedArray<T, kChunkSize> v;
        for (size_t i = 0; i < n; ++i)
            v.push_back(value);
        benchmark::DoNotOptimize(n > 0 ? &v[0] : nullptr);
        benchmark::ClobberMemory();
...
template <typename T>
static void BM_ChunkedArrayMerged_PushBack(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        mrg::ChunkedArrayMerged<T, kChunkSize> v;
        for (size_t i = 0; i < n; ++i)
            v.push_back(value);
        benchmark::DoNotOptimize(n > 0 ? &v[0] : nullptr);
        benchmark::ClobberMemory();
...
template <typename T>
static void BM_ChunkedArrayRecoil_RandomAccessRead(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    rec::ChunkedArray<T, kChunkSize> v;
    v.ensure_size(n);
    const std::vector<size_t> indices = MakeShuffledIndices(n);
...
template <typename T>
static void BM_ChunkedArrayMerged_RandomAccessRead(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    mrg::ChunkedArrayMerged<T, kChunkSize> v;
    v.ensure_size(n);
    const std::vector<size_t> indices = MakeShuffledIndices(n);
...
template <typename T>
static void BM_ChunkedArrayRecoil_RandomAccessWrite(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    rec::ChunkedArray<T, kChunkSize> v;
    v.ensure_size(n);
    T value{};
...
template <typename T>
static void BM_ChunkedArrayMerged_RandomAccessWrite(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    mrg::ChunkedArrayMerged<T, kChunkSize> v;
    v.ensure_size(n);
    T value{};
...
template <typename T>
static void BM_ChunkedArrayRecoil_Iteration(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    rec::ChunkedArray<T, kChunkSize> v;
    v.ensure_size(n);
    for (auto _ : state) {
...
template <typename T>
static void BM_ChunkedArrayMerged_Iteration(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    mrg::ChunkedArrayMerged<T, kChunkSize> v;
    v.ensure_size(n);
    for (auto _ : state) {
...
template <typename T>
static void BM_ChunkedArrayRecoil_ReserveThenPushBack(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        rec::ChunkedArray<T, kChunkSize> v;
...
template <typename T>
static void BM_ChunkedArrayMerged_ReserveThenPushBack(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        mrg::ChunkedArrayMerged<T, kChunkSize> v;
...
template <typename T>
static void BM_ChunkedArrayRecoil_EnsureSizeThenFill(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        rec::ChunkedArray<T, kChunkSize> v;
...
template <typename T>
static void BM_ChunkedArrayMerged_EnsureSizeThenFill(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    T value{};
    for (auto _ : state) {
        mrg::ChunkedArrayMerged<T, kChunkSize> v;
...
template <typename T>
static void BM_ChunkedArrayRecoil_Back(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    rec::ChunkedArray<T, kChunkSize> v;
    v.ensure_size(n);
    for (auto _ : state) {
...
template <typename T>
static void BM_ChunkedArrayMerged_Back(benchmark::State& state) {
    size_t n = static_cast<size_t>(state.range(0));
    mrg::ChunkedArrayMerged<T, kChunkSize> v;
    v.ensure_size(n);
    for (auto _ : state) {
...
#define REGISTER_TYPE_BENCHMARKS(Type) \
    ...
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_PushBack, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_PushBack, Type) \
    ...
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_RandomAccessRead, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_RandomAccessRead, Type) \
    ...
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_RandomAccessWrite, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_RandomAccessWrite, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_Vector_Iteration, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArray_Iteration, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_Iteration, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_Iteration, Type) \
    ...
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_ReserveThenPushBack, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_ReserveThenPushBack, Type) \
    ...
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_EnsureSizeThenFill, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_EnsureSizeThenFill, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_Vector_Back, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArray_Back, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayRecoil_Back, Type) \
    REGISTER_BENCHMARK_FOR_TYPE(BM_ChunkedArrayMerged_Back, Type)
