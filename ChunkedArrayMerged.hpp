...
namespace entable {
    ...
    private:
        using Helper = ChunkHelperMerged<CHUNK_SIZE>;
    ...
    public:
        ...
        class Iterator {
        private:
            using ArrayType     = std::conditional_t<IsConst, const ChunkedArrayMerged, ChunkedArrayMerged>;
        ...
        ChunkedArrayMerged() = default;

        ~ChunkedArrayMerged() {
            destroy_elements(0, elemCount);
        }

        ChunkedArrayMerged(const ChunkedArrayMerged&)            = delete;
        ChunkedArrayMerged& operator=(const ChunkedArrayMerged&) = delete;

        ChunkedArrayMerged(ChunkedArrayMerged&& other) noexcept
        ...
        ChunkedArrayMerged& operator=(ChunkedArrayMerged&& other) noexcept {
            if (this != &other) {
                destroy_elements(0, elemCount);
                chunks     = std::move(other.chunks);
                elemCount  = std::exchange(other.elemCount, 0);
        ...
        FORCE_INLINE decltype(auto) at(size_t idx) {
            if (idx >= elemCount) [[unlikely]] throw std::out_of_range("ChunkedArrayMerged::at index out of range");
            return (*this)[idx];
        }
        ...
        //   });
        //
        // Note: chunk spans must not outlive the ChunkedArrayMerged, and the array
